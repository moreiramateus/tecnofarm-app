...
type Props = {
  useContextHook: () => { [key: string]: any; set: (data: object) => void };
  field: string;
  title?: string;
  description?: string;
};

export default function MultiCameraInputGlobal({
  useContextHook,
  field,
  title = "Fotos",
  description = "Tire uma ou mais fotos",
}: Props) {
  const { [field]: value, set } = useContextHook();
